...
def dot_mod(left: Sequence[int], right: Sequence[int], p: int) -> int:
    if len(left) != len(right):
        raise ValueError(f"dot product length mismatch: {len(left)} != {len(right)}")
    return sum(int(a) * int(b) for a, b in zip(left, right)) % p


def transpose_matrix(matrix: Sequence[Sequence[int]]) -> List[List[int]]:
    if not matrix:
        return []
    col_count = len(matrix[0])
    for row in matrix:
        if len(row) != col_count:
            raise ValueError("cannot transpose ragged matrix")
    return [[int(matrix[row][col]) for row in range(len(matrix))] for col in range(col_count)]


def normalize_mod_vector(
    vector: Sequence[int],
    p: int,
    *,
    ...
    if preferred_index is not None:
        if preferred_index < 0 or preferred_index >= len(values):
            raise ValueError(f"preferred normalization index out of range: {preferred_index}")
        if values[preferred_index] % p:
            pivot = int(preferred_index)
        else:
            pivot = next(idx for idx, value in enumerate(values) if value % p)
    else:
        pivot = next(idx for idx, value in enumerate(values) if value % p)
    inv = fm.mod_inv(values[pivot], p)
    normalized = [value * inv % p for value in values]
    return normalized, {
        "normalization_index": pivot,
        "normalization_value_mod_p_before_scaling": str(values[pivot]),
        "normalization_value_mod_p_after_scaling": "1",
    }


def left_kernel_vector_from_selected_minor(
    *,
    row_count: int,
    selected_rows: Sequence[int],
    selected_columns: Sequence[int],
    column_vectors: Dict[int, List[int]],
    p: int,
    normalization_row: Optional[int] = None,
) -> Tuple[List[int], Dict[str, Any]]:
    rank = len(selected_columns)
    if len(selected_rows) != rank:
        raise ValueError("selected row/column counts differ")
    if row_count != rank + 1:
        raise ValueError(f"left-kernel helper expects corank one, got row_count={row_count}, rank={rank}")
    selected_row_set = {int(row_idx) for row_idx in selected_rows}
    if len(selected_row_set) != len(selected_rows):
        raise ValueError("selected rows are not unique")
    if any(row_idx < 0 or row_idx >= row_count for row_idx in selected_row_set):
        raise ValueError("selected row index out of range")
    missing_rows = [idx for idx in range(row_count) if idx not in selected_row_set]
    if len(missing_rows) != 1:
        raise ValueError(f"expected exactly one omitted row, got {missing_rows}")
    omitted_row = missing_rows[0]

    minor = selected_minor_matrix(selected_rows, selected_columns, column_vectors)
    det = determinant_mod(minor, p)
    if det % p == 0:
        raise ZeroDivisionError("selected corank-one minor is singular")
    rhs = [
        (-int(column_vectors[int(w_idx)][omitted_row])) % p
        for w_idx in selected_columns
    ]
    selected_coeffs = solve_square_mod(transpose_matrix(minor), rhs, p)
    vector = [0 for _ in range(row_count)]
    for row_idx, coeff in zip(selected_rows, selected_coeffs):
        vector[int(row_idx)] = int(coeff) % p
    vector[omitted_row] = 1
    normalized, norm_payload = normalize_mod_vector(
        vector,
        p,
        preferred_index=omitted_row if normalization_row is None else normalization_row,
    )
    norm_payload.update({
        "omitted_row_index": omitted_row,
        "selected_minor_determinant_mod_p": str(det % p),
        "construction": "Set the omitted row coefficient to 1, solve A^T x = -b for selected row coefficients, then scale.",
    })
    for w_idx in selected_columns:
        if dot_mod(normalized, column_vectors[int(w_idx)], p):
            raise RuntimeError("constructed vector failed on a selected certificate column")
    return normalized, norm_payload
